...
def classify_episode(
    *, success: bool, candidates: Iterable[dict[str, Any]], outcomes: Iterable[str] = ()
) -> dict[str, Any]:
    evidence = sorted(
        list(candidates),
        key=lambda item: (item.get("time_s") is None, item.get("time_s", float("inf"))),
    )
    ...
    valid = [
        ...
        if item.get("label") in LABELS and item.get("evidence")
    ]
    if not valid:
        ...
        label = (
            earliest[0]["label"]
            if len({item["label"] for item in earliest}) == 1
            else "other_uncertain"
        )
